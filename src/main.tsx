...
function Rotina(){
  ...
  const [done,setDone]=React.useState<number[]>([0,1,2,3])
            ...
            <Card title={`Crescimento — ${nome}`}>
              ...
              <div style={{marginTop:16,borderTop:`1px solid ${C.line}`,paddingTop:12}}>
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                  {[['Roupa',''],['Calçado',''],['Camiseta',''],['Calça','']].map(([k])=>(<div key={k}><label style={{fontSize:11,color:'rgba(255,255,255,.4)',display:'block',marginBottom:3}}>{k}</label><input placeholder="Ex: 12 anos / 34" style={{width:'100%',background:C.bg,border:'1px solid rgba(255,255,255,.15)',borderRadius:8,padding:'8px 10px',color:'#fff',fontSize:12}}/></div>))}
                </div>
              </div>
            </Card>
          </div>
...
function Exercicios(){const [ativo,setAtivo]=React.useState(false);const [inicio,setInicio]=React.useState(0);const [dur,setDur]=React.useState(0);const [saved,setSaved]=React.useState(false);React.useEffect(()=>{if(!ativo)return;const t=setInterval(()=>setDur(Math.floor((Date.now()-inicio)/1000)),1000);return()=>clearInterval(t)},[ativo,inicio]);const fmt=(s:number)=>`${String(Math.floor(s/60)).padStart(2,'0')}:${String(s%60).padStart(2,'0')}`;return(<div style={{padding:'24px 28px'}}><h1 style={{fontSize:24,fontWeight:800,marginBottom:4}}>Exercícios</h1><p style={{color:'rgba(255,255,255,.4)',fontSize:13,marginBottom:20}}>Plano semanal · domingo é descanso</p><div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:10,marginBottom:20}}>{[['Seg','Calistenia',true],['Ter','Caminhada',true],['Qua','Calistenia',true],['Qui','Caminhada',false],['Sex','Calistenia',false],['Sáb','Mobilidade',false],['Dom','Descanso',false]].map(([d,t,done])=>(<div key={String(d)} style={{background:C.s2,border:`1px solid ${C.line}`,borderRadius:12,padding:'12px 8px',textAlign:'center' as const}}><div style={{fontSize:11,color:'rgba(255,255,255,.4)',marginBottom:6}}>{d}</div><div style={{fontSize:12,fontWeight:600,marginBottom:6}}>{t}</div>{done&&<span style={{color:C.ok}}>✓</span>}</div>))}</div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}><Card title="Treino de hoje">{saved&&<div style={{background:'rgba(52,211,153,.1)',border:'1px solid rgba(52,211,153,.3)',borderRadius:10,padding:'10px 12px',fontSize:13,color:C.ok,marginBottom:12}}>✓ Treino salvo! {fmt(dur)}</div>}{ativo&&<div style={{background:'rgba(139,92,246,.15)',border:'1px solid rgba(139,92,246,.35)',borderRadius:12,padding:'20px',textAlign:'center' as const,marginBottom:12}}><div style={{fontSize:36,fontWeight:800,fontFamily:'monospace'}}>{fmt(dur)}</div><div style={{fontSize:13,color:'rgba(255,255,255,.6)',marginTop:4}}>em andamento…</div></div>}{!saved&&<button onClick={()=>{if(!ativo){setAtivo(true);setInicio(Date.now())}else{setAtivo(false);setSaved(true)}}} style={{width:'100%',background:ativo?`linear-gradient(135deg,${C.ok},#15803d)`:`linear-gradient(135deg,${C.acc},#7c3aed)`,color:'#fff',border:'none',borderRadius:10,padding:'14px',fontSize:15,fontWeight:700,cursor:'pointer',marginBottom:10}}>{ativo?`⏹ Parar (${fmt(dur)})`:'▶ Iniciar treino'}</button>}</Card><Card title="Histórico">{[['28/07','Calistenia','45 min'],['27/07','Caminhada','40 min'],['26/07','Calistenia','50 min'],['25/07','Caminhada','35 min']].map(([d,t,dr])=>(<div key={String(d)} style={{display:'flex',gap:8,padding:'9px 0',borderBottom:`1px solid ${C.line}`,fontSize:13,color:'rgba(255,255,255,.6)'}}><span style={{width:40}}>{d}</span><span style={{flex:1}}>{t}</span><span>{dr}</span></div>))}</Card></div></div>)}
