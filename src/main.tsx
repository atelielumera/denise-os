...
function Casa(){
  const [items,setItems]=React.useState<{n:string,cat:string,done:boolean}[]>(()=>{try{return JSON.parse(localStorage.getItem('dos_casa_items')||'null')||[{n:'Iogurte proteico sem lactose',cat:'Mercado',done:false},{n:'Whey isolado',cat:'Mercado',done:false},{n:'Frango',cat:'Mercado',done:false},{n:'Limpar casa',cat:'Doméstico',done:false},{n:'Pagar energia',cat:'Contas',done:false}]}catch{return []}})
  const [nova,setNova]=React.useState('')
  const [cat,setCat]=React.useState('Mercado')
  const cats=['Mercado','Doméstico','Manutenção','Contas']
  function salvarItems(n:{n:string,cat:string,done:boolean}[]){setItems(n);localStorage.setItem('dos_casa_items',JSON.stringify(n))}
  function addItem(){if(!nova)return;salvarItems([{n:nova,cat,done:false},...items]);setNova('')}
  function toggleItem(item:{n:string,cat:string,done:boolean}){salvarItems(items.map(x=>x===item?{...x,done:!x.done}:x))}
  function delItem(item:{n:string,cat:string,done:boolean}){salvarItems(items.filter(x=>x!==item))}
  return(<div style={{padding:'24px 28px'}}><h1 style={{fontSize:24,fontWeight:800,marginBottom:4}}>Casa</h1><p style={{color:'rgba(255,255,255,.4)',fontSize:13,marginBottom:20}}>Compras, tarefas, manutenção e contas</p><div style={{display:'flex',gap:10,marginBottom:20,flexWrap:'wrap' as const}}><input value={nova} onChange={e=>setNova(e.target.value)} onKeyDown={e=>e.key==='Enter'&&addItem()} placeholder="Adicionar item…" style={{flex:1,minWidth:200,background:C.s2,border:`1px solid ${C.line}`,borderRadius:10,padding:'10px 14px',color:'#fff',fontSize:14}}/><select value={cat} onChange={e=>setCat(e.target.value)} style={{background:C.s2,border:`1px solid ${C.line}`,borderRadius:10,padding:'10px 14px',color:'#fff',fontSize:14,colorScheme:'dark'}}>{cats.map(c=>(<option key={c}>{c}</option>))}</select><button onClick={addItem} style={{background:`linear-gradient(135deg,${C.acc},#7c3aed)`,color:'#fff',border:'none',borderRadius:10,padding:'10px 16px',fontSize:13,fontWeight:700,cursor:'pointer'}}>+ Adicionar</button></div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>{cats.map(c=>(<div key={c}><Card title={c}>{items.filter(i=>i.cat===c).map((item,idx)=>(<div key={idx} style={{display:'flex',alignItems:'center',gap:10,padding:'9px 0',borderBottom:`1px solid ${C.line}`,opacity:item.done?0.5:1}}><span onClick={()=>toggleItem(item)} style={{width:22,height:22,borderRadius:6,border:`2px solid ${item.done?C.ok:'rgba(255,255,255,.2)'}`,background:item.done?'rgba(52,211,153,.2)':'transparent',display:'grid',placeItems:'center',color:C.ok,fontSize:12,flexShrink:0,cursor:'pointer'}}>{item.done&&'✓'}</span><span onClick={()=>toggleItem(item)} style={{flex:1,fontSize:13,textDecoration:item.done?'line-through':'none',color:item.done?'rgba(255,255,255,.4)':'#fff',cursor:'pointer'}}>{item.n}</span><button onClick={()=>delItem(item)} style={{background:'rgba(248,113,113,.1)',border:'none',color:C.danger,borderRadius:6,padding:'3px 8px',fontSize:11,cursor:'pointer',flexShrink:0}}>✕</button></div>))}{items.filter(i=>i.cat===c).length===0&&<div style={{fontSize:13,color:'rgba(255,255,255,.3)',padding:'10px 0'}}>Nenhum item</div>}</Card></div>))}</div></div>)}
